...
atFacultyChoose.command(faculties.map(el => el.title), ctx => {
    ...
    getAddr(chosen_one.href, addr => {
        fs.writeFileSync('./data/addr.txt', addr);
    })
    if (!isForNews) {
        return Reply.text(ctx.data.request.command + "? Отличный выбор! Вот, что я могу рассказать о нём:\n\n",
            {
                buttons: [
                    ...
                    {title: "Проходные баллы прошлых лет",hide: true},                 
                    {title: "Время обучения",hide: true},
                    {title: "Количество бюджетных и платных мест",hide: true},
...
atFacultyChoose.command(/(Контакты)|(адрес)/i, ctx => {
    let addr = fs.readFileSync('./data/addr.txt', 'utf-8');

    return Reply.text(addr);
});
atFacultyChoose.command(/(Узнать последние новости)|(давай)|(что нового)|(нов)/i, ctx => {
    ...
    let temp = [];
    if (jsonNews.length <= 5)
        for (let i = 0; i < jsonNews.length; i++) {
            if (jsonNews[i].url.includes("https")) {
    ...
    let itemsN = temp.map(el => ({
        image_id: "1656841/987aad4849a6e123fb01",
        title: el.description.substring(0, 100),
        description: el.date_day,
        button: {
...
atFacultyChoose.command(/(Проходные баллы прошлых лет)|(баллы за 2018)/i, ctx => {
    if (chosen_one.points)
        return Reply.text("Минимальные проходные баллы по ЕГЭ на \"" + chosen_one.title + "\" за 2018 год:\n " + chosen_one.points, {
            buttons: [     
                {title: "Время обучения",hide: true},
                {title:"Количество бюджетных и платных мест",hide: true},
                {title: "Контакты",hide: true},
...
atFacultyChoose.command(/(Количество бюджетных и платных мест)|(мест[а])|(бюджет)|(плат[ка]*)/, ctx => {
    let counts = chosen_one.positions.split('/');
    return Reply.text("Бюджетных мест: " + counts[0] + "\nПлатных: " + counts[1], {
        buttons: [
            {title:"Контакты",hide: true},
            {title:"Новости",hide: true},
            {title:"Цена за обучение",hide: true},
...
atFacultyChoose.command(/(Назад)|(верн[и])|(стой)|(наверх)|(ой)/i, ctx => {
    return Reply.text(chosen_one.title + "?\nВот, что я могу рассказать об образовательной программе:\n\n",
    {
        buttons: [
            {title: "Цена за обучение",hide: true},
            {title: "Минимальные проходные баллы",hide: true},
...
atFacultyChoose.command(/(выйти)|(уйти)|(пока)|(досвидания)|(выход)/i, ctx => {
    ctx.leave();
    return Reply.text("Хотите узнать новости или информацию о поступлении?", {
        buttons: [Markup.button({ title: "О поступлении", hide: true }),
        Markup.button({ title: "Новости", hide: true })
